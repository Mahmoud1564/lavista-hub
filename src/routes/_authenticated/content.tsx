  ...
  return (
    <div className="space-y-4">
      <Card className="p-4 bg-muted/30 text-xs text-muted-foreground">
        Contact info, footer, and social links are managed in{" "}
        <span className="text-primary font-medium">Settings</span> — single source of truth.
      </Card>
      {SECTIONS.map((s) => (
        <SectionCard
          key={s.key}
          ...
          renderExtras={
            s.key === "about"
              ? (row, onChange) => <AboutImage row={row} onChange={onChange} />
              : undefined
          }
        />
      ))}
...
function SectionCard({
  ...
  useEffect(() => {
    ...
    section.fields.forEach((f) => {
      initial[f.name] = (row?.value?.[f.name] as string) ?? "";
    });
    setValues(initial);
  }, [row, section]);
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({ key: section.key, value: merged }, { onConflict: "key" });
      if (error) throw error;
  ...
  return (
    <Card className="p-5">
      ...
      {renderExtras && (
        <div className="mt-4 pt-4 border-t border-border">{renderExtras(row, onSaved)}</div>
      )}
      <div className="mt-4">
...
function AboutImage({ row, onChange }: { row: ContentRow | null; onChange: () => void }) {
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const merged = { ...(row?.value ?? {}), image_url: newPath } as unknown as Record<
        string,
        string
      >;
      const { error } = await supabase
        .from("website_content")
        .upsert({ key: "about", value: merged }, { onConflict: "key" });
      if (error) throw error;
    ...
    <div>
      ...
      <div className="flex items-start gap-4 mt-2">
        <div className="w-40 h-28 bg-muted rounded-md overflow-hidden flex items-center justify-center flex-shrink-0">
          {signed ? (
            <img src={signed} alt="About" className="w-full h-full object-cover" />
          ) : (
