...
function FaqPage() {
  ...
  async function add() {
    ...
    const { error } = await supabase.from("faq").insert({ question: q, answer: a, sort_order: order });
    if (error) toast.error(error.message); else { toast.success("Added"); setQ(""); setA(""); setAdding(false); refresh(); }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={() => setAdding(!adding)}><Plus className="w-4 h-4" />{adding ? "Cancel" : "Add FAQ"}</Button>
      </div>

      {adding && (
        <Card className="p-5 space-y-3">
          <Input placeholder="Question" value={q} onChange={(e) => setQ(e.target.value)} />
          <Textarea rows={3} placeholder="Answer" value={a} onChange={(e) => setA(e.target.value)} />
          <Button onClick={add}>Save FAQ</Button>
        </Card>
      )}

      {items.length === 0 ? <Card className="p-8"><Empty title="No FAQs yet" /></Card> : (
            ...
            <Card key={f.id} className="p-5">
              <div className="flex items-start gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  <button onClick={() => move(i, 1)} disabled={i === items.length - 1} className="text-muted-foreground hover:text-foreground disabled:opacity-30"><ChevronDown className="w-4 h-4" /></button>
                </div>
                <div className="flex-1 space-y-2">
                  <Input defaultValue={f.question} onBlur={(e) => e.target.value !== f.question && update(f.id, { question: e.target.value })} />
                  <Textarea defaultValue={f.answer} rows={2} onBlur={(e) => e.target.value !== f.answer && update(f.id, { answer: e.target.value })} />
                </div>
                <Button size="sm" variant="ghost" onClick={() => del(f.id)}><Trash2 className="w-4 h-4" /></Button>
              </div>
            </Card>
          ))}
        </div>
