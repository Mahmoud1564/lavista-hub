...
function MediaPage() {
  ...
  async function del(name: string) {
    if (
      !confirm("Delete this image? Any room/experience/review still using it will lose its photo.")
    )
      return;
  ...
  return (
    <div className="space-y-4">
      ...
      {files.length === 0 ? (
        <Card className="p-8">
          <Empty title="No images in this bucket yet" />
        </Card>
      ) : (
          ...
          {files.map((f) => (
            <Card key={f.name} className="overflow-hidden group">
              ...
              <div className="p-2 flex items-center justify-between gap-1">
                <span className="text-xs text-muted-foreground truncate flex-1" title={f.name}>
                  {f.name}
                </span>
                <Button
                  size="sm"
