...
type Review = {
  ...
  image_url: string | null; is_visible: boolean;
};
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={() => setCreating(true)}><Plus className="w-4 h-4" />Add review</Button>
      </div>
      {reviews.length === 0 ? <Card className="p-8"><Empty title="No reviews yet" /></Card> : (
        <div className="grid sm:grid-cols-2 gap-4">
          {reviews.map((r) => <ReviewCard key={r.id} r={r} onEdit={() => setEditing(r)} onToggle={() => toggle(r)} onDelete={() => del(r)} />)}
        </div>
      )}
...
function ReviewCard({ r, onEdit, onToggle, onDelete }: { r: Review; onEdit: () => void; onToggle: () => void; onDelete: () => void }) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            <Badge variant={r.is_visible ? "success" : "muted"}>{r.is_visible ? "Visible" : "Hidden"}</Badge>
          </div>
...
function Form({ item, onSaved }: { item?: Review; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const payload = { guest_name: guestName, rating, review_text: text || null, image_url: imagePath, is_visible: isVisible };
      if (isEdit) {
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          <div className="w-16 h-16 rounded-full bg-muted overflow-hidden flex items-center justify-center">
            {imgUrl ? <img src={imgUrl} alt="" className="w-full h-full object-cover" /> : <Upload className="w-5 h-5 text-muted-foreground" />}
          </div>
