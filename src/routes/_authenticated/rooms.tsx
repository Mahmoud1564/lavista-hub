...
function RoomsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {rooms.length === 0 ? (
        <Card className="p-8">
          <Empty title="No rooms yet" hint="Click 'Add room' to get started." />
        </Card>
      ) : (
            ...
            <Card key={r.id} className="overflow-hidden">
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
                  <span>
                    <span className="text-primary font-semibold">${r.price}</span> / night
                  </span>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => deleteRoom(r)}>
...
function RoomForm({ room, onSaved }: { room?: Room; onSaved: () => void }) {
    ...
    (async () => {
      ...
      setImages(
        (imgs ?? []).map((i) => ({
          ...
          signed: signedMap[i.image_url] ?? null,
  ...
  function addCustomAmenity() {
    ...
    if (!n || amenities.some((a) => a.amenity === n)) {
      setCustomAmenity("");
      return;
    }
      ...
      if (roomId) {
        if (isEdit) {
          ...
          const toDelete = (current ?? [])
            .filter((c) => !targetNames.has(c.amenity))
            .map((c) => c.id);
          const toInsert = amenities
            .filter((a) => !currentNames.has(a.amenity))
            .map((a) => ({ room_id: roomId!, amenity: a.amenity }));
          ...
          await supabase
            .from("room_amenities")
            .insert(amenities.map((a) => ({ room_id: roomId!, amenity: a.amenity })));
        }
      }
    ...
    <div className="space-y-4">
      ...
      <div className="pt-4 border-t border-border">
          ...
          {images.map((img) => (
            ...
            >
              ...
              <button
                ...
                className={`absolute top-1 left-1 rounded-full p-1 ${img.is_thumbnail ? "bg-primary text-primary-foreground" : "bg-background/70 text-foreground opacity-0 group-hover:opacity-100"}`}
              >
        ...
        {!isEdit && pending.length > 0 && (
          <p className="text-[10px] text-muted-foreground">
            {pending.length} image{pending.length === 1 ? "" : "s"} will upload when you create the
            room.
          </p>
        )}
      </div>
      ...
      <div className="pt-4 border-t border-border">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 mb-3">
          {PREDEFINED_AMENITIES.map((a) => (
            <label
              key={a}
              className={`flex items-center gap-2 text-xs px-2 py-1.5 rounded-md cursor-pointer border ${selectedAmenities.has(a) ? "bg-primary/15 border-primary text-foreground" : "border-border hover:bg-accent"}`}
            >
              <input
                type="checkbox"
        ...
        {amenities.filter(
          (a) => !PREDEFINED_AMENITIES.includes(a.amenity as (typeof PREDEFINED_AMENITIES)[number]),
        ).length > 0 && (
          <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {amenities
                .filter(
                  (a) =>
                    !PREDEFINED_AMENITIES.includes(
                      a.amenity as (typeof PREDEFINED_AMENITIES)[number],
                    ),
                )
                .map((a) => (
                  <span
      ...
      <div className="pt-4 border-t border-border">
        <Button onClick={save} disabled={saving}>
          {saving ? "Saving..." : isEdit ? "Save room" : "Create room"}
        </Button>
      </div>
    </div>
  );
